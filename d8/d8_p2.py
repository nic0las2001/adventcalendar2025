...
iii = 0
cond2 = True 
while len(circuits) > 1 or cond2:
    iii += 1
    if iii % 100 == 0:
        print(iii, "step, circuits:", len(circuits))
    # find min circuit
    min = 0
    ...
    if min != 0: 
        ...
        last_connection = (min[0], min[1])
    if len(circuits) > 0:
        cond2 = len(circuits[0]) < n_box

print("P2", junction_boxes[last_connection[0]][0]*junction_boxes[last_connection[1]][0])

'''Very slow method as we parse through everything so many times the bruteforce way, 
next time worth sorting the list of calculated distances as it's quicker than looking 
for the smallest number every time through 1000s of coordinates'''
